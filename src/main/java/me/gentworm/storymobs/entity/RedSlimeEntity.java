...
public class RedSlimeEntity extends MobEntity implements IMob {
	private static final DataParameter<Integer> SLIME_SIZE = EntityDataManager.createKey(RedSlimeEntity.class,
	...
	public RedSlimeEntity(EntityType<? extends RedSlimeEntity> type, World worldIn) {
		super(type, worldIn);
		this.moveController = new RedSlimeEntity.MoveHelperController(this);
	}
	...
	public void tick() {
		...
		if (this.onGround && !this.wasOnGround) {
			...
			for (int j = 0; j < i * 8; ++j) {
	            ...
	            this.world.addParticle(new ItemParticleData(ParticleTypes.ITEM, new ItemStack(Blocks.REDSTONE_BLOCK)), this.getPosX() + (double) f2, this.getPosY(), this.getPosZ() + (double) f3, 0.0D, 0.0D, 0.0D);
	...
	public void remove(boolean keepData) {
		...
		if (!this.world.isRemote && i > 1 && this.getShouldBeDead() && !this.removed) {
			...
			for (int l = 0; l < k; ++l) {
				...
				RedSlimeEntity slimeentity = this.getType().create(this.world);
				...
				slimeentity.setLocationAndAngles(this.getPosX() + (double) f1, this.getPosY() + 0.5D,
	...
	protected void dealDamage(LivingEntity entityIn) {
		if (this.isAlive()) {
			int i = this.getSlimeSize();
			if (this.getDistanceSq(entityIn) < 0.6D * (double) i * 0.6D * (double) i && this.canEntityBeSeen(entityIn)
	...
	public static AttributeModifierMap.MutableAttribute getAttributes() {
		return MobEntity.func_233666_p_().createMutableAttribute(Attributes.ATTACK_DAMAGE, 6.0D)
	...
	static class AttackGoal extends Goal {
		...
		public boolean shouldExecute() {
			...
			if (livingentity == null) {
				...
				return livingentity instanceof PlayerEntity && ((PlayerEntity) livingentity).abilities.disableDamage
						? false
						: this.slime.getMoveHelper() instanceof RedSlimeEntity.MoveHelperController;
			}
		}
		...
		public void tick() {
			...
			((RedSlimeEntity.MoveHelperController) this.slime.getMoveHelper()).setDirection(this.slime.rotationYaw,
	...
	static class FaceRandomGoal extends Goal {
		...
		public void tick() {
			...
			((RedSlimeEntity.MoveHelperController) this.slime.getMoveHelper()).setDirection(this.chosenDegrees, false);
		}
	}
	...
	static class FloatGoal extends Goal {
		...
		public void tick() {
			...
			((RedSlimeEntity.MoveHelperController) this.slime.getMoveHelper()).setSpeed(1.2D);
		}
	}
	...
	static class HopGoal extends Goal {
		 ...
		 */
		public void tick() {
			((RedSlimeEntity.MoveHelperController) this.slime.getMoveHelper()).setSpeed(1.0D);
		}
	}
